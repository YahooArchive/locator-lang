...
When it comes to lang specific, you can do the following, which produce a json object with the specific lang entries collected by the plugin, in this case, accessing the BCP 47 language tag `fr-FR`:

* http://localhost:3000/demo/foo?lang=fr-FR

You can access to the static yui module in a particular language as well. Keep in mind that for the url and the module name we use lowercase version of the BCP 47 language tag to work with certain CDN providers:
